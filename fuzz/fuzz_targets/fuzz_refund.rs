...
struct Input {
    ...
    /// Partial/no/full funding before the deadline, mirroring fuzz_release.
    pay_amount: i64,
    /// Explicit before/after-deadline toggle, independent of
    /// `deadline_offset_secs`, so both "called before deadline" (must
    /// reject with "deadline has not passed") and "called after deadline"
    /// (must succeed / no-op depending on funding) are directly reachable
    /// regardless of how the offset randomizes.
    call_after_deadline: bool,
    /// Raw invoice id passed to `refund` — deliberately not clamped so most
    /// inputs target nonexistent invoices and exercise `InvoiceNotFound`.
    invoice_id: u64,
    /// Call `refund` a second time on the same post-refund state to probe
    /// double-refund handling.
    call_twice: bool,
}
...
fuzz_target!(|input: Input| {
    ...
    // The invariant under test: refunding any invoice id — real or
    // nonexistent, funded or not, before or after its deadline — must
    // return exactly the paid-in balance to payers (never more), must never
    // corrupt storage, and must only panic through a documented
    // assert!/panic! guard.
